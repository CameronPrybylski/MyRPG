...
int main()
{
    ...
    if(!app.Create("Game", 1067, 800)){
        ...
        std::shared_ptr<Battle> battle = std::make_shared<Battle>(app.GetScreenWidth(), app.GetScreenHeight(), (std::string(root) + "/areas/battle.json").c_str(), (std::string(root) + "/savestate/saveBattle.json").c_str(), (std::string(root) + "/savestate/saveGame.json").c_str(), std::string(root));
